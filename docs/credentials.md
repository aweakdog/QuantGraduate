...
| `QUANT_OPS_PASSWORD` | `611611` | **所有会写状态的操作**：改名、切记账方式、确认成交、校准现金、存取现金、删持仓、重置、手动跑信号、人工对账；另外还有**访问日志**查询 | 12 小时 |
| `QUANT_BT_PASSWORD` | `Llyh19990710` | 隐藏的回测页 `/backtest`（首页不链接到它） | 7 天 |

三道口令**互不通用**：签名密钥掺入了不同的用途字符串，所以回测页的 cookie
当不了写权限用。能看不等于能改——即使已登录，改账仍要单独输 ops 口令。
