...
def main():
    # 1. Load existing events_clean (缺失时退化为空基底, 全靠公告关键词生成)
    if EVENTS_CLEAN.exists():
        ev_clean = pd.read_parquet(str(EVENTS_CLEAN))
    else:
        print(f"[EVENTS_CLEAN] 不存在, 使用空基底: {EVENTS_CLEAN}")
        ev_clean = pd.DataFrame(columns=["code", "name", "event_type", "p_level",
                                         "direction", "date", "title", "snippet"])
    existing_titles = set(ev_clean["title"].dropna().unique()) if len(ev_clean) else set()
    existing_keys = set(zip(ev_clean["code"], ev_clean["title"].fillna(""))) if len(ev_clean) else set()

    nst = ev_clean['code'].nunique() if len(ev_clean) else 0
    print(f"[EVENTS_CLEAN] {len(ev_clean)} rows, {nst} stocks, {len(existing_titles)} unique titles")
