...
def build_all(incremental: bool = True, max_workers: int = 16) -> None:
    ...
    log.info("训练特征: %d 只, incremental=%s, max_workers=%d", len(stocks), incremental, max_workers)
    ...
    if all_dfs:
        ...
        train_path = os.path.join(DATA_DIR, "processed", "training_data_v15.parquet")
        train.to_parquet(train_path, index=False)
        log.info("训练集 v15: %d 行, %d 列, max=%s",
                 len(train), len(train.columns), train["date"].max())
    else:
        log.warning("无有效特征数据")
