...
def parse_md(answer):
    ...
    for l in lines[hidx + 1:]:
        ...
        # 兜底: 限流导致 iFinD 返回降级 schema(缺开盘/收盘列)时,
        # 用前收盘价近似同日收盘/开盘, 保证不被丢弃
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="iFinD kline daily backfill")
    ap.add_argument("--start", default="2026-07-01", help="query start YYYY-MM-DD")
    ap.add_argument("--end", default="2026-07-10", help="query end YYYY-MM-DD")
    ap.add_argument("--test", type=int, default=0, help="limit to first N stocks")
    ap.add_argument("--threshold", default=None, help="backfill stocks with max < this date (default=end)")
    ap.add_argument("--progress", action="store_true", help="output PROGRESS:N/M lines for UI")
    args = ap.parse_args()
    progress = args.progress
    threshold = pd.Timestamp(args.threshold or args.end)
    ...
    for c in codes:
        ...
        if not os.path.exists(p):
            stuck.append(c); continue
        d = pd.read_parquet(p)
        t = d["时间"] if "时间" in d.columns else d.iloc[:, 0]
        mx = pd.to_datetime(t, errors="coerce").max()
        if pd.isna(mx) or mx < threshold:
            stuck.append(c)
    print(f"universe={len(codes)} stuck={len(stuck)}", flush=True)
    if test_n:
        stuck = stuck[:test_n]
    sid = new_session()
    if not sid:
    ...
    if progress:
        print(f"PROGRESS:{done}/{len(stuck)}", flush=True)
    for c in stuck:
        q = f"{to_ifind(c)} {start_str}至{end_str}的前复权每日开盘价、最高价、最低价、收盘价、成交量、成交额"
        rows = None
        for attempt in range(3):
            ...
            time.sleep(2)
        if not rows:
            print(f"FAIL {c} ans={str(ans)[:600]}", flush=True); fail += 1
            if progress:
                print(f"PROGRESS:{done + fail}/{len(stuck)}", flush=True)
        ...
        print(f"OK {c} +{len(new)} rows -> max {comb['时间'].max().date()}", flush=True)
        if progress:
            print(f"PROGRESS:{done + fail}/{len(stuck)}", flush=True)
        time.sleep(0.3)
    print(f"DONE done={done} fail={fail}", flush=True)
