"""
R6 — fund=main_force_pct@50%, tech调低
"""
...
print("R6 — fund=pct@50%% 标的: %s" % STOCK)
...
print("=== R6 fund=main_force_pct@50% ===")
...
# 总分: event35 + fund50 + tech15
df["total_score"] = event_score * 0.35 + fund_score * 0.50 + tech_score * 0.15
...
for i in range(len(df)):
    if df["position"].iloc[i] == 1.0 and not in_position:
        ...
        loss_entry = (curr - entry_p) / entry_p; loss_peak = (curr - peak_p) / peak_p
        atr = df["atr14"].iloc[i] / 100.0
        if atr < 0.01: atr = 0.01
        stop1 = loss_entry < -2.0 * atr; stop2 = loss_peak < -1.5 * atr and loss_entry > 0.03
...
result = {"stock": stock, "version": "R6_pct50",
          "weights": "event35_fund50_pct_tech15"}
