...
BASE_PARAMS = {
    ...
    # 窗 B' 配对 -54.2pp, 0/12 正 —— 2022-09 之前的旧历史把 alpha 打到零.
    # 但固定 3 年滑窗不是答案: 09-06 PLR3 用逐字生产配置(四条主板线, 20 种子, 同种子配对)
    # 测 --train-years 3 vs expanding: aggr5w -15.4pp(5/20, p=.04) / steady5w -12.0(3/20, p=.003)
    # / aggr10w -11.4 / fyf100w -4.4 —— 丢掉 2022-09~2023-09 那一年是丢营养, 毒药边界在
    # 2022-09 之前. 生产矩阵本来就从 2022-09-01 起, 所以 None(expanding) 就是当前最优已知.
    # 管道留着不删(不进指纹, 改了也只换模型不动持仓账): 2027-09 矩阵满 5 年时重测 4y 滑窗.
    # 证据 docs/findings_2026-09-05_expand_2019_prodlineage.md §3.2/§3.4
    "train-years": None,
