...
BASE_PARAMS = {
    ...
    "regime-filter": "off",
    # 整手粒度救济: 槽位预算买不起一手时, 若一手成本 <= 预算*1.5 仍买进,
    # 而不是沿排名换下一只。机制上只在"买不起一手"触发时才改变行为:
    # 拒单越频繁的配置受益越大, 几乎不触发的配置不受影响 (2026-08-02 A/B,
    # 基于 preds_CS80, 6配置×flex{0,0.25,0.5}):
    #   2万/3只 (157次拒单): IR 0.62 -> 1.41, 两段都改善 (0.64->0.76, 0.64->1.86)
    #   5万/10万 (1~46次):  ΔIR 在 ±0.08 内, 噪声水平
    # 副作用: 单槽最大超配 50% (n3 时权重 33%->42%), 下次换仓配平拉回。
...
PROFILES = {
    "steady2w": {
        ...
        "capital": 20000.0,
        "tranche-n": 3,
        "desc": "2万本金下可行的最分散方案。回测 IR 1.41, 回撤 -27.4%, 两段都跑赢 (flex0.5)。",
    },
    "aggr2w": {
        "name": "激进 2万",
        "capital": 20000.0,
        "tranche-n": 2,
        "desc": "只持 2 只, 每只预算更大所以高价股也能买。回测 IR 1.29 但仅 2 只集中度最高 (flex0.5)。",
    },
    "steady5w": {
        "name": "稳妥 5万",
        "capital": 50000.0,
        "tranche-n": 5,
        "desc": "5 只分散, 单股爆雷影响最小。回测 IR 1.44, 回撤 -36.8% (flex0.5)。",
    },
    "aggr5w": {
        "name": "激进 5万",
        "capital": 50000.0,
        "tranche-n": 3,
        "desc": "回测指标最优 (IR 1.54, 回撤 -28.3%, flex0.5), 但仅 3 只, 个股集中度高。",
    },
    # ── 不可更改的基准线 ───────────────────────────
