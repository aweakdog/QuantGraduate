"""
Walk-Forward 回测 v27: fwd_1d_ret (close→close) + DART boosting
标签: fwd_1d_ret = close_{T+1} / close_T - 1 (T收盘买, T+1收盘卖)
...
parser = argparse.ArgumentParser(description="WF v27 close-to-close DART")
...
print(f"Label: {LABEL_RAW} (close_T → close_{TEST_START and 'T+1'})")
...
# Execution simulation with close-price
...
print(f"\n{'='*60}")
print(f"  v27 Close-to-Close DART Results")
print(f"{'='*60}")
...
output = {
    ...
    "model": f"LightGBM DART n={LOCKED_PARAMS['n_estimators']} d={LOCKED_PARAMS['max_depth']} "
             f"lr={LOCKED_PARAMS['learning_rate']}, n_jobs={LOCKED_PARAMS['n_jobs']}, close-to-close",
