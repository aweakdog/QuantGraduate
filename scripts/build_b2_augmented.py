...
────────────────────────────────────────────────────────────────────
  ...
  第二波 (矩阵 tick1_b3 = tick1_b2 + 9 列; 不变性检验: 同 80 列同种子 b2/b3 结果逐位相同)
  T2C ❌ 5 种子 +24.9pp 3/5 过门 -> 20 面板 -4.0pp, 9/20, p=0.75 完全反转 (已知待解禁行仅 1.5%)
  T2B ❌ 5 种子 +0.7pp -> 20 面板 +1.25pp, 11/20, p=0.41 wash (池内每日中位 3 只上榜)
  T3H ❌ 5 种子不过门 (中位 -14.0pp, 1/5; 季频慢变量, 与 FDX1 同死法)
六族都不进生产。本文件保留的价值: (1) 判决可复现; (2) 载入器可复用 —— load_kline
(全市场 qfq 长表) / margin_detail / sw_l1_daily (申万一级 PIT 区间贴标签) / share_float
(ann_date 次日已知) / top_inst 席位聚合 / stk_holdernumber (混合日期格式 + merge_asof PIT),
新族加一个 frame 函数 + ALL_FAMS 一行即可挂进来。
